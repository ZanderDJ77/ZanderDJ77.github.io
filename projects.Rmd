...
To make data more manageable, a small dataset was created containing 3 Sites of interest across the Geography of NY State, 2006 - 2010, which have a monitoring point every day from 2006-2010.
...
```{r}
full_tempchange = full_nyweather_df |> 
  mutate(month = lubridate::floor_date(date, unit = "month")) |> 
  drop_na(tmax) |>
  group_by(id) |> 
  mutate(temp_change = tmax - lag(tmax)) |> 
  summarize(sd_temp_change = sd(temp_change, na.rm = TRUE)) |> 
  mutate(rank = min_rank(sd_temp_change)) |> 
  arrange(rank)

plot_ly(full_tempchange, x = ~id, y = ~sd_temp_change, color = ~id, type= "bar")
```

```{r}
snowfall = full_nyweather_df |> 
  select(id, date, snow) |> 
  drop_na(snow) |> 
  mutate(rank = min_rank(desc(snow))) |> 
  filter(rank >= 51)

plot_ly(snowfall, x = ~date, y = ~snow, color = ~id, type= "bar")
...
```{r}
temperature_change = weather_sites_df |> 
...
plot_ly(temperature_change, x= ~month, y = ~sd_temp_change, color = ~name, type= "bar", mode = "markers")
```
